// SpotGhost Background Service Worker
// Handles caching, notifications, security checks, and data management
...
async function updateScamCompaniesDatabase() {
  try {
    const settings = await chrome.storage.local.get(['apiEndpoint']);
    const apiUrl = `${settings.apiEndpoint || 'http://localhost:3000'}/api/scam-companies`;
    
    const response = await fetch(apiUrl);
    const scamCompanies = await response.json();
    
    if (response.ok && Array.isArray(scamCompanies)) {
      await chrome.storage.local.set({
        [SCAM_COMPANIES_CACHE_KEY]: scamCompanies,
        lastDatabaseUpdate: Date.now()
      });
      console.log(`Updated scam companies database: ${scamCompanies.length} entries`);
    }
  } catch (error) {
